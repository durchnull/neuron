...
class OrderAddCartRuleAction extends OrderAction
{
    public static function rules(): array
    {
        return [
            'order_id' => 'required|uuid|exists:orders,id',
            'cart_rule_id' => 'required|exists:cart_rules,id,enabled,1'
        ];
    }
    ...
    protected function apply(): void
    {
        ...
        $this->target->cartRules()->attach($cartRule);
        $this->target->setRelation(
            'cartRules',
            collect([$cartRule])->concat($this->target->cartRules->where('id', '!=', $cartRule->id))
        );
